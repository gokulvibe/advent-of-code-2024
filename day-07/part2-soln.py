...
def find_if_result(result, input_nums, curr_index, curr_result):
    if curr_result == result and curr_index == len(input_nums):
        return True
    if curr_index>=len(input_nums) or curr_result>result:
        return False

    return find_if_result(result, input_nums, curr_index+1, curr_result + input_nums[curr_index]) \
            or find_if_result(result, input_nums, curr_index+1, curr_result * input_nums[curr_index])\
            or find_if_result(result, input_nums, curr_index+1, int(str(curr_result) + str(input_nums[curr_index])))
